...
class REM:
    def __init__(self, args, obs_dim, action_dim):
        ...
        # Q net.
        self.q_net = Qnet_Multihead(self.obs_dim, self.action_dim, self.num_nets).to(self.device)

        # Target Q net.
        self.target_q_net = Qnet_Multihead(self.obs_dim, self.action_dim, self.num_nets).to(self.device)
    ...
    def update(self, transition_dict):
        ...
        alpha = torch.rand(self.num_nets).to(self.device)
        alpha = alpha / torch.sum(alpha)
        alpha = alpha.unsqueeze(-1).expand(-1, len(actions))  # [num_head, batch]

        # Get Q(s,a) for actions taken
        actions = actions.unsqueeze(-1).expand(self.num_nets, -1, -1)
        q_values = self.q_net(states).gather(2, actions).squeeze()
        q_values = torch.sum(alpha * q_values, dim=0)  # [batch]

        # Get Q-target
        with torch.no_grad():
            next_q_values = self.target_q_net(next_state)  # [num_head, batch, action_dim]
            next_q_values = torch.sum(next_q_values * alpha.unsqueeze(-1).expand(-1, -1, self.action_dim),
                                      dim=0)  # [batch, action_dim]
            next_q_values, _ = torch.max(next_q_values, dim=1)  # [batch]
            next_q_values[dones] = 0
        q_targets = (next_q_values * self.discount_rate) + rewards
...
def train_REM_agent(args, seed):
    ...
    # ========================= Build DQN Network. ========================= #
    agent = REM(args, observation_dim, action_dim)
